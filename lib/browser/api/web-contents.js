...
WebContents.prototype.executeJavaScript = function (code, hasUserGesture, callback) {
  ...
  // 4. Then executeJavaScript() is called on the newly created WebContents
  ...
  // 5. this.isLoadingMainFrame() should be true because there is no main frame for it
  if (this.getURL() && !this.isLoadingMainFrame()) {
    console.log("WebContents.executeJavaScript")
    return asyncWebFrameMethods.call(this, requestId, 'executeJavaScript', callback, code, hasUserGesture)
  } else {

    // 6. And really, there's nobody calling emitting the event DidFinishLoad() for this WebContents instance :(
    console.log("WebContents.executeJavaScript waiting")

    return new Promise((resolve, reject) => {
      this.once('did-frame-finish-load', () => {
        console.log("WebContents.executeJavaScript did-frame-finish-load")
      })
      this.once('did-finish-load', () => {
        console.log("WebContents.executeJavaScript did-finish-load")
